...
(function (window) {
    ...
    const KEYS = { REF: 'wit_ref', SID: 'wit_sid' };
    ...
    const getVitals = (isInitial) => {
        ...
        return {
            ...
            webVitals: isInitial ? {
                ...
                lcp: Math.round(sessionState.lcp) + 'ms',
    ...
    new PerformanceObserver(l => { for (const e of l.getEntries()) sessionState.longTasks.push({ d: Math.round(e.duration) + 'ms' }); }).observe({ type: 'longtask', buffered: true });
    ...
    const Witnes = {
        identify: (id) => { 
            identifiedUserId = id; 
            if (id && document.readyState === 'complete') Witnes.emit('LOAD'); 
        },
        emit: (eventType) => {
            const pk = config.projectKey || config.project_key;
            if (!pk || !identifiedUserId) return;
            ...
            const endpoint = `http://localhost:7070/api/v1/ingestion?pk=${pk}`;
    ...
    // --- 5. Event Listeners ---
    window.addEventListener('load', () => setTimeout(() => Witnes.emit('LOAD'), 500));
    const onNav = () => {
        if (window.location.href !== lastUrl) {
            lastUrl = window.location.href;
            setTimeout(() => Witnes.emit('SPA_NAV'), 800);
        }
    };
